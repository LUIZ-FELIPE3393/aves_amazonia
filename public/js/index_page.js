...
axios.get('bird').then(json => {
    for (const bird of json.data) {
        ...
        const birdElement = document.createElement('a')
        birdElement.setAttribute('class', 'flex bg-lime-100 shadow rounded cursor-pointer')
        birdElement.setAttribute('bird-id', json['bdt_id'])
        birdElement.setAttribute('href', `/artigo-ave?id=${bird['bdt_id']}`)
        ...
        document.querySelector('#bird-gallery').appendChild(birdElement)
    }
})